...
# Groeptags definiëren voor eenvoudige aanpassing
$groupTags = @{
    ...
    "Q" = @{ Name = "Afsluiten"; GroupTag = ""; Color = "Yellow"; Description = "Sluit het script af" }
}
...
function Display-Menu {
    ...
    do {
        ...
        if ($choice -eq 'H') {
            Show-Help -groupTags $groupTags
        } elseif ($choice -eq 'Q') {
            Write-Host "Script wordt afgesloten..." -ForegroundColor Cyan
            exit
        } elseif ($groupTags.ContainsKey($choice)) {
            Get-AutopilotInfo -GroupTag $groupTags[$choice].GroupTag
            Write-Host "`nDruk op ENTER om terug te keren naar het menu..." -ForegroundColor Cyan
            Read-Host
        } else {
            Write-Host "`n!! FOUT: Ongeldige invoer. Voer een geldige optie in. !!" -ForegroundColor Red
